...
clean: env-down
	...
	@docker rm -f -v `docker ps -a --no-trunc | grep "secc-service" | cut -d ' ' -f 1` 2>/dev/null || true
	@docker rmi `docker images --no-trunc | grep "secc-service" | cut -d ' ' -f 1` 2>/dev/null || true
	@echo "Clean up done"
